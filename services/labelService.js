...
// ─── Impressão ────────────────────────────────────────────────────────────────
// Dois modos, escolhidos por dispositivo (na tela) ou por env:
//   • 'network'  → envia o ZPL direto p/ Zebra de rede via TCP raw 9100.
//                  Funciona de qualquer servidor que ALCANCE o IP da impressora.
//   • 'windows'  → manda o ZPL bruto para uma impressora COMPARTILHADA do Windows
//                  (copy /B p/ \\host\compartilhamento). É o modo do app antigo e
//                  o único que funciona para impressora USB compartilhada (ex.:
//                  ELGIN/Zebra ligada na máquina e compartilhada como ZEBRA_RAW1).
//                  Exige que o backend rode no Windows que enxerga o compartilhamento.

const DEFAULT_MODE = (process.env.LABEL_PRINTER_MODE || (process.env.LABEL_PRINTER_NAME ? 'windows' : 'network')).toLowerCase();

export function isPrinterConfigured() {
  return Boolean(process.env.LABEL_PRINTER_HOST || process.env.LABEL_PRINTER_NAME);
}

// Despacha p/ o modo certo. opts: { mode, host, port, name } — tudo opcional;
// o que vier vazio cai para o env.
export async function printZpl(zpl, opts = {}) {
  const mode = (opts.mode || DEFAULT_MODE).toLowerCase();
  if (mode === 'windows') {
    return sendZplToWindowsPrinter(zpl, { name: opts.name, host: opts.host });
  }
  return sendZplToPrinter(zpl, { host: opts.host, port: opts.port });
}

// Modo rede (TCP raw 9100).
export async function sendZplToPrinter(zpl, override = {}) {
  const host = (override.host && String(override.host).trim()) || process.env.LABEL_PRINTER_HOST;
  const port = Number(override.port || process.env.LABEL_PRINTER_PORT || 9100);
  if (!host) throw new Error('Impressora de rede não configurada (defina o IP na tela ou LABEL_PRINTER_HOST no .env).');
  ...
  return new Promise((resolvePromise, rejectPromise) => {
    ...
    socket.on('error', (err) => rejectPromise(err));
    socket.on('timeout', () => { socket.destroy(); rejectPromise(new Error('Timeout ao conectar na impressora.')); });
    socket.on('close', () => resolvePromise({ mode: 'network', printer: `${host}:${port}` }));
  });
}

// Nome de compartilhamento aceitável (evita injeção no copy/UNC).
function assertSafeShareName(name) {
  if (!/^[\w .$()+#-]{1,80}$/.test(name)) {
    throw new Error('Nome de impressora/compartilhamento inválido.');
  }
}

// Host UNC válido (nome de máquina ou IP). Sem ':', '\\', '/' — esses geram o
// erro críptico "A sintaxe do nome do arquivo... está incorreta" no copy.
function assertSafeUncHost(host) {
  if (!/^[A-Za-z0-9._-]{1,255}$/.test(host)) {
    throw new Error(`Host de impressora inválido para compartilhamento do Windows: "${host}". Use só o nome da máquina ou IP (sem ":porta").`);
  }
}

// Modo Windows (impressora compartilhada). Grava um .zpl temporário e faz
// `copy /B <tmp> \\host\compartilhamento`, exatamente como o app standalone.
export async function sendZplToWindowsPrinter(zpl, override = {}) {
  const shareName = String(override.name || process.env.LABEL_PRINTER_NAME || '').trim();
  const host = String(override.host || process.env.LABEL_PRINTER_HOST_WIN || 'localhost').trim() || 'localhost';
  if (!shareName) throw new Error('Impressora do Windows não configurada (informe o nome de compartilhamento na tela ou LABEL_PRINTER_NAME no .env).');
  if (process.platform !== 'win32') throw new Error('Impressão por compartilhamento do Windows só funciona com o backend rodando no Windows.');
  assertSafeShareName(shareName);
  assertSafeUncHost(host);

  const { mkdtemp, writeFile, rm } = await import('node:fs/promises');
  const os = await import('node:os');
  const { execFile } = await import('node:child_process');

  const dir = await mkdtemp(resolve(os.tmpdir(), 'orq-zpl-'));
  const tmp = resolve(dir, `label-${Date.now()}.zpl`);
  await writeFile(tmp, zpl, 'latin1');

  try {
    await new Promise((resolvePromise, rejectPromise) => {
      execFile(
        'cmd.exe',
        ['/c', 'copy', '/B', tmp, `\\\\${host}\\${shareName}`],
        { windowsHide: true, timeout: 15000 },
        (error, stdout, stderr) => {
          if (error) return rejectPromise(new Error((stderr || error.message || '').toString().trim() || 'Falha no spool do Windows.'));
          return resolvePromise();
        },
      );
    });
    return { mode: 'windows', printer: `\\\\${host}\\${shareName}` };
  } finally {
    rm(dir, { recursive: true, force: true }).catch(() => {});
  }
}

// Lista as impressoras instaladas no Windows (para o seletor da tela).
// Só funciona com o backend no Windows; em outro SO devolve [].
export async function listWindowsPrinters() {
  ...
  return new Promise((resolvePromise) => {
    execFile(
      ...
      (error, stdout) => {
        if (error || !stdout) return resolvePromise([]);
        try {
          const parsed = JSON.parse(String(stdout).replace(/^﻿/, '').trim());
          const arr = Array.isArray(parsed) ? parsed : [parsed];
          resolvePromise(arr.map((p) => ({
            name: String(p.Name || ''),
